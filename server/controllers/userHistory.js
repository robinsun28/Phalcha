...
export const addHistory = tryCatch(async (req, res) => {
  ...
  const newHistory = new UserHistory({ userId, roomId });    //adds mew history
...
export const getHistories = tryCatch(async (req, res) => {
  const userId = req.user.id;
  const histories = await UserHistory.find({ userId }).sort({ _id: -1 });   //fetches the rooms interacted by the user
